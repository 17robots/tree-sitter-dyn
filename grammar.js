...
module.exports = grammar({
  ...
  rules: {
    ...
    arm: $ => seq(choice($.expression, '_'), ':', optional($.capture), choice($.expression, $.block, seq($.while_prefix), seq($.for_prefix), seq($.if_prefix)), ','), // TODO: this needs finished

    statement: $ => choice(
      prec(1, $.match),
      seq($.while_prefix, choice($.block, seq($.statement, ';'))),
      seq($.for_prefix, choice($.block, seq($.statement, ';'))),
      seq($.if_prefix, choice($.block, seq($.statement, ';')), optional(seq('else', choice($.block, $.statement)))),
      seq($.expression, ';'),
    ),
    ...
    optional_type: $ => prec(1, seq('?', $.type_expression)),
    pointer_type: $ => prec.right(1,seq('*', $.type_expression)),
    array_type: $ => prec.right(1, seq('[', ']', $.type_expression)),
    fn_type: $ => prec.right(seq('(', commaSep($.type_expression), ')', optional($.type_expression))),
    ...
    comp_type: $ => seq('comp', $.type_expression),
