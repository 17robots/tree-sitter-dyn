...
module.exports = grammar({
  ...
  rules: {
    ...
    struct: $ => seq(optional(token('packed')), token('struct'), $.identifier, '{', commaSep($.struct_member), '}'),
    struct_member: $ => seq($.identifier, repeat(seq(',', $.identifier)), $.type_qualifier, optional(seq('=', $.expression))),
    enum: $ => seq(token('enum'), $.identifier, optional(seq('(', choice($.primitive, $.field_type), ')')), '{', commaSep($.enum_member), '}'),
    ...
    statement: $ => choice($.return_, $.continue_, $.break_, $.variable, $.thread_local_variable, $.const_variable, $.if_, $.case_, $.for_, $.defer, $.expression, $.assignment, $.panic, $.syscall),
    ...
    type: $ => prec.right(choice($.field_type, $.array_type, $.pointer_type, $.primitive, $.fn_type)),
    ...
    fn_type: $ => prec.right(seq(token('fn'), '(', commaSep($.type), ')', optional($.type))),
    expression: $ => choice($.literal, $.primary, $.unary_prefix, $.binary, $.size, $.cast, $.len, $.align),
    ...
    index: $ => prec(PREC.POSTFIX, seq($.primary, '[', choice($.expression, seq('..', $.expression), seq($.expression, '..')), ']')),
    size: $ => seq(token('#sizeof'), '(', $.type, ')'),
    align: $ => seq(token('#alignof'), '(', $.type, ')'),
    len: $ => seq(token('#len'), '(', $.type, ')'),
    cast: $ => seq(token('#cast'), '(', $.type, ')', $.expression),
    literal: $ => choice($.bool_, $.null_, $.number_, $.string_, $.char_, $.struct_literal, $.array_literal),
    ...
    array_literal: $ => seq('[', choice($.number_, '_'), ']', $.type, '{', commaSep($.expression), '}'),
